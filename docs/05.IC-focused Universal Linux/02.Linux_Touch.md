...
```bash
/            （根目录，整个文件系统起点）		/bin         （基础可执行命令，系统最小运行集）  
/sbin        （系统管理命令）  				/etc         （系统配置文件）  
/home        （普通用户目录）  				/root        （root 用户主目录）  
/usr         （大部分应用程序与库文件） 	 	 /var         （可变数据，如日志）  
/proc        （内核状态映射，虚拟文件系统）    /dev         （设备文件接口）  
/boot        （内核与启动文件）  		    /tmp         （临时文件）  
/lib /lib64  （基础共享库）
```
